...
class DocumentService:
    def create_original_and_revised_docs(
        self, reviewed_data: dict, out_dir: Path, blob_name: str
    ) -> tuple[Path, Path]:
        """
    ...
    def create_final_document_with_bubbles(self, reviewed_data: dict, output_docx: str) -> None:
        """
    ...
    def _add_comment_bubble_opc(
        ...
        try:
            ...
            tree_c.write(comments_path, xml_declaration=True, encoding="UTF-8", standalone=True)
            ...
            if para_index < len(paras):
                ...
                cref = etree.SubElement(ref_run, f"{{{NS_W}}}commentReference", nsmap=NSMAP_W)
